...
function compile({ rootDirectory, files, logger }) {
  ...
  for (const file of files) {
    ...
    switch (fileExt) {
      ...
      case ".js":
      case ".jsx": {
        const { code } = transformSync(readFileSync(file).toString(), {
          filename: fileName,
        });
        let distFileName = fileName;
        if (fileExt === ".tsx" || fileExt === ".ts") {
          distFileName = distFileName.replace(fileExt, ".js");
        }
        writeFileSync(pathJoin(distPath, distFileName), code);
        compileCount++;
        break;
      }
      default: {
        copyFileSync(file, pathJoin(distPath, fileName));
        copyCount++;
        break;
      }
    }
  ...
  logger.log(
    `compiled: ${compileCount} file${compileCount === 1 ? "" : "s"}${copyCount > 0 ? `, copied ${copyCount} file${copyCount === 1 ? "" : "s"}` : ""}`,
  );
