...
import path, { basename, dirname, extname, join as pathJoin } from "node:path";
  ...
  for (const filePath of files) {
    ...
    switch (extname(filePath)) {
      ...
      case ".tsx": {
        ...
        if (declaration) {
          writeFileSyncWithDirs(distPath.replace(/(\.tsx|\.ts)/, ".d.ts"), declaration);
        }
        compileCount++;
        break;
      }
...
export async function runBuild({ rootDirectory, logger } = { rootDirectory: process.cwd(), logger: console }) {
  ...
  logger.log(
    `compiled: ${compileCount} file${compileCount === 1 ? "" : "s"}${
      copyCount > 0 ? `, copied ${copyCount} file${copyCount === 1 ? "" : "s"}` : ""
    }`,
  );
  try {
